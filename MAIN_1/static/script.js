...
function stopDrawing(container) {
  ...
  if (container.id === "code-container") {
    drawnBoxes.code = currentBox.style; // Store code box
  } else if (container.id === "graph-container") {
    drawnBoxes.graph = currentBox.style; // Store graph box
  }
}

// Attach events for starting, updating, and stopping drawing
...
function saveBoxesToDatabase(boxes) {
  // Remove 'px' suffix and convert values to integers
  ...
  fetch("/start_eyetracker", {
    ...
    .then((data) => {
      if (data.status === "success") {
        ...
        showPopupMessage("The eye-tracker has started", 2000); // Display the pop-up for 2 seconds
        listenForGazeData();
      } else {
...
function getSectionOffsets() {
  ...
  // Calculate offsets for each container
  const codeOffset = {
  ...
  // Log the offsets to see their values
...
function updateGazeDuration(matchedBoxId) {
  ...
  if (gazeDurations[matchedBoxId].duration >= timeThreshold) {
    drawBoxesForMatchedId(matchedBoxId); // Draw the red box
    delete gazeDurations[matchedBoxId]; // Reset to avoid multiple draws
  }
}

function pollForUpdates() {
  fetch("/get_latest_match") // Use the existing endpoint to get the latest match
    .then((response) => response.json())
    .then((data) => {
      if (data.status === "success" && data.match) {
        ...
        if (latestBoxId === currentMatchedBoxId) {
          // Gaze continues on the same AOI
          updateGazeDuration(latestBoxId);
        } else {
          // Gaze shifted to a new AOI, reset durations
